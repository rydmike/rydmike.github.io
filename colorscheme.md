...
scheme to this example. New in this example is also that the surface and background colors have all received a
...
made above and one more olive green one defined from just one color per theme mode, despite that it has automatic
nuances too. Surface and background branding level is a bit lower in this example than in the pervious one,
it is set to medium.
...
certainly works dandy on a phone too. This version can do a lot of the things seen on the **Theme** page in 
the **Flexfold demo** app. It even has fake semi responsive menu/rail included, so that it can have a bit more of 
the branded surfaces visible. 

This example can even do a trick that the Flexfold demo cannot, it can switch to use the same ColorScheme 
data that it has defined, and instead of creating the theme with its own "toTheme" method, it can do it with the
standard "ThemeData.from" color scheme factory. If you open two versions of the example 5 app, side by side in 
two browser-windows, you can compare the results, not quite the same thing! FlexSchemeColors is working a bit of
theming magic in comparison.
...
As mentioned, the **Flexfold** web demo app also uses the **FlexColorScheme** package for all of its theming.
...
and go to the **Theme** page. If you select custom theme, custom surface and custom app bar, you can pretty much build
any kind of theme you want interactively using [**FlexColorPicker**](https://rydmike.com/colorpicker) to pick colors.

So where do the funny theme names come from? Like Mandy red, Hippie blue among other. I actually used Flexfold's
customizable FlexColorScheme mode to design some of the themes in FlexColorScheme, and when you do that
**FlexColorPicker** can name the chosen color automatically, sometimes the color names where just too hip to not 
use in the theme names.
