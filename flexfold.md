| [Home](README) | Flexfold             | [Color Scheme](colorscheme) | [Color Picker](colorpicker) |  
| [Talo](talo)   | [Grid](gridview)     | [Issues](issues)            | [Experiments](experiments)  |

## Flexfold

Flexfold is a responsive scaffold package for Flutter. A live **web demo** of an **example app** using the 
**Flexfold package** can be seen [**tested here**](http://rydmike.com/demoflexfold). It is my intention to
...
not yet available on Flutter's stable channel. To avoid releasing separate versions, one that would work on the 
stable channel and another one for beta/dev/master channels, it is being held back until the APIs land in 
the stable channel too. **OK, you got me**, it's the darn docs that I have not had time to complete. 
...
defined responsive behavior can be also tested and observed on different device approximations on the Web version.
...
all the settings have tooltips that explain the API used to control each setting.
...
that show the impact of some of its key settings.
...
Last but not least of the wild things, is a toggle that enables **background plasma** effect on the side bar 
and Flexfold Home page.
...
The Plasma effect I used are just defined in code and tied to the primary theme color and light/dark
